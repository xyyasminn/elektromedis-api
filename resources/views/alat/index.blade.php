...
<head>
    ...
    <style>
        ...
        .subtitle{color:#7f8c8d;font-size:13px;margin-bottom:10px}
        a.add-btn{display:inline-block;margin-bottom:15px;padding:8px 14px;background:#2ecc71;color:#fff;text-decoration:none;border-radius:5px;font-size:14px}
        form{margin-bottom:15px}
        input,select,button,a.reset{padding:6px 10px;font-size:13px;margin-right:5px}
        a.reset{text-decoration:none;border:1px solid #ccc;color:#333;background:#eee}
...
<div class="container">
    <div class="header">
        ...
        <a href="{{ route('alat.create') }}" class="add-btn">+ Tambah Alat Baru</a>
    </div>
    ...
    <form method="GET">
        ...
        <a href="{{ route('alat.index') }}" class="reset">Reset</a>
    </form>
    ...
    <table>
        ...
        @forelse($data as $i=>$alat)
            <tr>
                ...
                <td>
                    @if($alat->gambar)
                        <img src="{{ asset('storage/'.$alat->gambar) }}" class="thumb">
                    @else
                        -
                    @endif
                </td>
